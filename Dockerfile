...
RUN mkdir /build/cpython-3.7
RUN tar xvf /build/cpython.tar.gz -C /build/cpython-3.7 --strip-components 1
...
RUN \
	...
	echo "exec $PREFIX/bin/python3 -m nvpy "'"$@"'                >>/usr/local/bin/nvpy && \
